...
# ---------------------------------------------------------
#  User Configuration (Roboflow Dataset)
# ---------------------------------------------------------
_C.NUM_CLASSES = 2  # 1 Person + 1 Background

# Add the 'params' dictionary that your modified train.py expects
_C.params = EasyDict()
_C.params.img_train_path = './dataset/roboflow/train/'
_C.params.img_val_path = './dataset/roboflow/valid/'


# ---------------------------------------------------------
#  Model Architecture & Anchors (Do Not Change)
# ---------------------------------------------------------
...
_C.img_mean = np.array([0., 0., 0.])[:, np.newaxis, np.newaxis].astype('float32')
_C.resize_width = 640
...
# Legacy face config (kept to prevent errors in other utils, though not used)
_C.FACE = EasyDict()
